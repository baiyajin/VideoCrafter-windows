...
class Text2Video():
    ...
    def get_prompt(self, prompt, steps=50, cfg_scale=12.0, eta=1.0, fps=16):
        ...
        print('='*60)
        print(f'[Text2Video] 开始生成视频')
        print(f'[Text2Video] 提示词: {prompt}')
        print(f'[Text2Video] 参数: steps={steps}, cfg_scale={cfg_scale}, fps={fps}')
        print(f'[Text2Video] 时间: {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))}')
        print('='*60)
        start = time.time()
        gpu_id=0
        if steps > 60:
        ...
        print(f'[Text2Video] 噪声形状: {noise_shape}')

        # text cond
        print(f'[Text2Video] 开始文本编码...')
        text_start = time.time()
        text_emb = model.get_learned_conditioning([prompt])
        cond = {"c_crossattn": [text_emb], "fps": fps}
        print(f'[Text2Video] 文本编码完成，耗时: {time.time() - text_start:.2f}秒')
        
        ## inference
        print(f'[Text2Video] 开始DDIM采样...')
        ...
        print(f'[Text2Video] 开始保存视频...')
        save_start = time.time()
        save_videos(batch_samples, self.result_dir, filenames=[prompt_str], fps=self.save_fps)
        print(f'[Text2Video] 视频保存完成，耗时: {time.time() - save_start:.2f}秒')
        print(f'[Text2Video] 总耗时: {time.time() - start:.2f}秒')
        print('='*60)
        model=model.cpu()
