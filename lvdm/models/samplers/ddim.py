...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        print(f"[DDIM采样] 开始循环，总步数: {total_steps}")
        for i, step in enumerate(iterator):
            if i % 10 == 0 or i == 0:  # 每10步输出一次，或第一步
                elapsed = time.time() - loop_start
                print(f"[DDIM采样] 步骤 {i+1}/{total_steps}, timestep={step}, 已耗时: {elapsed:.2f}秒")
            ...
            if i % 10 == 0 or i == 0:
                print(f"[DDIM采样] 步骤 {i+1} 推理完成，耗时: {step_time:.2f}秒")
